...
Overseerr's API returns title-cased status strings (`"Processing"`, `"Not Requested"`, `"Partially Available"`) which conflict with the lowercase enum expected by `display_titles` (`"pending"`, `"not_requested"`, `"partial"`). `normalizeMediaStatus()` in `overseerr.ts` handles the mapping. The `overseerr_search` and `overseerr_discover` tool handlers apply this function before returning results so the LLM always sees display_titles-compatible values. `enrichSonarrSeries()` in `sonarr-tools.ts` also pre-computes `mediaStatus` — `"available"` when found in Plex, normalized Overseerr status when found there, otherwise `"partial"` if monitored by Sonarr (being managed — no request button) or `"not_requested"` if not. `searchSeries()` in `sonarr.ts` queries `/series` (the library-only endpoint) and filters results client-side by title substring or exact year; it never calls `/series/lookup`, which returns external TVDB results with `monitored=true` by default regardless of library membership.
